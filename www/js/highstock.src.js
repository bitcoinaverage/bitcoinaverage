...
RangeSelector.prototype = {
	...
	clickButton: function (i, rangeOptions, redraw) {
        ...
        selectedRangeValue = rangeSelector.selected;
        var currentHash = window.location.hash;
        currentHash = currentHash.replace('#', '');
        var data  = currentHash.split('-');
        var currencyCode = data[0];
        var chartMode = data[1];

        selectedRange = keys(selectedRangeValue);
        var currentLocation = document.location.href;
        var newLocation = currentLocation.replace(window.location.hash, '') + '#' + currencyCode + '-' + chartMode + '-' + selectedRange;
	 ...
	 */
    rangeSelectorCallback : function(chart){
