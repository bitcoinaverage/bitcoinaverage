...
var renderGlobalAverageData = function(apiData, currency)
{
   ...
   var template = $('#global-average-data-table tbody tr');
   //console.log(template);
   var tbody;

   var array = $.map(apiData, function(value, index) {
    value['currency'] = index;
    return [value];
   });

   array.splice(-2,2); // delete timestamp and ignored_exchanges from data
   array.sort(orderByVolume);
   ...
   $.each(array, function(i, item) {
       ...
       var crossPrice     = (fiatCurrencies[currencyCode]['rate'] / fiatCurrencies[currencyCode]['rate']) * lastPrice;
       crossPrice = crossPrice.toFixed(2);
       var row = template.clone();
       row.attr('id', 'currency-name-' + currencyCode);
       row.find('.legend-currency span').text(currencyCode);
       row.find('.legend-volume_percent span').text(volumePercent);
       row.find('.legend-volume_btc span').text(volumeBtc);
       row.find('.legend-price span').text(lastPrice);
       row.find('.legend-last-cross-price span').text(crossPrice);
       row.find('.legend-last-cross-price .legend-curcode').text(currencyCode);
